...
from .serializers import MedicationSerializer, ClinicMedicationSerializer, PatientMedicationSerializer, MedicationLogSerializer
...
def clinic_medications_list(request, clinic_id):
    ...
    cm, created = ClinicMedication.objects.get_or_create(clinic=clinic, medication=medication)
    return Response(
...
def clinic_medication_detail(request, clinic_id, medication_id):
    try:
        cm = ClinicMedication.objects.get(clinic_id=clinic_id, medication_id=medication_id)
...
@api_view(['GET', 'POST'])
@permission_classes([IsAuthenticated])
def patient_medications_list(request, clinic_id, patient_id):
    try:
        clinic = Clinic.objects.get(id=clinic_id)
    ...
    if not ClinicMedication.objects.filter(clinic=clinic, medication=medication).exists():
        return Response(
    ...
    serializer = PatientMedicationSerializer(data={
        **request.data,
...
def medication_log_detail(request, clinic_id, patient_id, medication_record_id, log_id):
    try:
        log = MedicationLog.objects.get(
            id=log_id, patient_medication_id=medication_record_id, patient__id=patient_id
        )
...
def patient_all_medication_logs(request, clinic_id, patient_id):
    ...
    ).select_related('patient_medication__medication').order_by('-taken_at')
    return Response(MedicationLogSerializer(qs, many=True).data)
